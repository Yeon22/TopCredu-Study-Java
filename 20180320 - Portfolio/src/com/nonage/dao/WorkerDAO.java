...
public class WorkerDAO {
	...
	//사용자 인증을 위한 메소드 : -1(아이디 존재 X) 0(아이디 존재하지만 비밀번호 불일치) 1(아이디와 비밀번호 모두 일치)
	//login.jsp -> workerCheck.jsp
	
	//아이디를 검색 조건을 주어서 비밀번호를 얻어온다.
	public int workerCheck(String userid, String userpw) {
		...
		try {
			...
			if(rs.next()) { //전달인자로 준 아이디와 일치하는 행이 존재
				result = 0; //등록된 관리자
				String dbPwd = rs.getString(1); //디비 저장된 비밀번호, rs.getString(1):첫번채 컬럼을 가져와라
