function NumFormat(t){ //원 단위 ,(컴마) 찍어주기
	s = t.value;
	s = s.replace(/\D/g,'');
	l = s.length - 3;
	while(l > 0){
...
function go_ab(){ //판매가-원가=순매출울 replace해서 계산해준다.
...
function removeComma(input){ //,(컴마)를 빼고 값을 다시 넣어준다.
	return input.value.replace(/,/gi, '');
}

//폼에 입력이 올바른지 판단한다.
//pfoductWrite.jsp에서 사용한다.
function go_save(){
	var theForm = document.frm;
	if(theForm.kind.value == ''){
		...
		//productWrite.jsp 폼 페이지에서 입력받은 값을 디비에 추가하기 위한 페이지인 productWrite.jsp로 이동하되
		//입력받은 상품코드를 쿼리 스트링 형태로 전달한다.
...
function go_detail(tpage, pseq){
	var theForm = document.frm;
	//상품 상세 보기 페이지에서 다시 상품 리스트로 돌아왔을 경우 현재 페이지로 돌아올 수 있도록 하기 위해서 현재 페이지 번호를 쿼리 스트링으로 넘겨준다.
...
function go_mod_save(tpage, pseq){
	...
	if(theForm.kind.value == ''){
		...
		if(confirm("수정하시겠습니까?")){
			//상품을 삭제하지 않는 대신 사용하지 않음을 products 테이블의 useyn컬럼에 1을 채워넣기 위해서 useyn hidden 태그에 1을 지정한다.
			...
			theForm.encoding = "multipart/form-date";
			//theForm.seq.value=seq;
			theForm.price1.value = removeComma(theForm.price1);
			theForm.price2.value = removeComma(theForm.price2);
			theForm.price3.value = removeComma(theForm.price3);
			//products테이블의 상품 정보를 수정하는 처리를 하는 product_modsave.jsp로 이동하되 상품코드를 전단해준다.
			//상품코드로 폴더를 생성해서 그곳에 이미지 파일을 업로드한다.
